...
export const cleanMediaQueryRule = (rule: string): string => {
	return rule.replace(REGEX_MEDIA_QUERY_BRACKETS_WITH_SPACE, (_, p) =>
		p !== undefined ? p : "",
	);
};

export const buildMediaTokens: BuildMediaTokensFunction = ({
	css,
	mediaQueries,
}) => {
	...
	for (const mediaQuery of mediaQueries) {
		...
		const value = css
			.slice(mediaQueryFirstParenthesis + 1, mediaQueryEnd - 1)
			.trim();
...
const getRuleValue: GetRuleValueFunction = ({ currentRules, newRules }) => {
	...
	return Object.entries(currentRulesObject)
...
export const tokenize = (css: string, optmizations?: Optimizations): Tokens => {
	...
	for (let index = 0; index < css.length; index++) {
		...
		if (
			...
			mediaQueryParsed < mediaQueries.length &&
