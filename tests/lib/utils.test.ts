...
describe("getOptimizations", () => {
	it.each(
		generateOptimizationFlagCases(),
	)("optimizeShortHand: %s, optimizeAll: %s, optimizeMargin: %s, optimizePadding: %s, removeZeroUnits: %s, expected: %s", (optimizeColor: boolean, optimizeShortHand: boolean, optimizeAll: boolean, optimizeMargin: boolean, optimizePadding: boolean, removeZeroUnits: boolean, expected: boolean[]) => {
		const options = {
			output: "output.css",
			optimizeShortHand,
			optimizeAll,
...
function generateOptimizationFlagCases(): Array<
...
> {
	...
	for (let i = 0; i < 32; i++) {
		...
		ret.push([
			...
			optimizePadding,
			removeZeroUnits,
			[expectedMargin, expectedPadding, expectedRemoveZeroUnits],
		]);
	}
	return ret;
}
