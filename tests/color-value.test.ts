...
describe("Color Value Test", () => {
	...
	it("preserves background with URL containing hash", () => {
		const css =
			".c { background: url('https://example.com/image.png?color=#000000');}";
		const result = compile(css);

		expect(result).toContain(
			"background:url('https://example.com/image.png?color=#000000')",
		);
	});
	...
	it("process colors when optimizeColors is true", () => {
		const css =
			".a { color: #ffffff; background-color: #000000; border: 1px solid #ff0000; }";
	...
	it("process colors and preserve correct spacing", () => {
		const css =
			".a { box-shadow: 0 0 5px 0     #ff0000;  background-color:#000000; color: #ffffff;}";
		const result = compile(css, { optimizeColors: true });
